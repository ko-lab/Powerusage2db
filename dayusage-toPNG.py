...
c = conn.cursor()
for row in c.execute("select * from powerusage where Timestamp >= Datetime('now', '-24 hours');"):
    L1.append(row[1])
    L2.append(row[2])
    L3.append(row[3])
...
plt.suptitle("Power consumption per phase last day")
...
plt.savefig("/var/www/vloer.ko-lab.space/verbruikdag.png")
